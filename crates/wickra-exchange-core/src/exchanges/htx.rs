...
impl Htx {
    ...
    fn get(&self, path: &str, query: &str) -> Result<serde_json::Value> {
        ...
        unwrap_status(&response.body).map_err(|e| {
...
mod tests {
    ...
    fn a_blocked_reply_is_reported_as_the_block_not_as_drift() {
        ...
        let error = htx.ticker(&symbol()).unwrap_err();
    ...
    fn debug_reports_state_without_credentials() {
        ...
        assert!(rendered.starts_with("Htx {"));
    ...
    fn rate_limit_carries_the_venues_advised_wait() {
        ...
        mock.push_response(
            crate::transport::HttpResponse::new(
                200,
                r#"{"status":"error","err-code":"rate-limit","err-msg":"slow"}"#,
            )
            .with_header("Retry-After", "2.5"),
        );
        let err = htx.ticker(&symbol()).unwrap_err();
